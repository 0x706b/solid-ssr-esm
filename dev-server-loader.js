...
export async function resolve(specifier, context, nextResolve) {
  let result;
  try {
    result = await tsNodeEsmHooks.resolve(specifier, context, nextResolve);
  } catch {
    result = await tsNodeEsmHooks.resolve(`${specifier}.jsx`, context, nextResolve);
  }
...
export async function load(url, context, defaultLoad) {
  ...
  if (format === "module" || format === "commonjs") {
    const config   = Babel.loadPartialConfig({ presets: [["solid", { generate: "ssr", hydratable: true }]] });
